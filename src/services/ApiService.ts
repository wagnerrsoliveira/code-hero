...
export class ApiService {
  private mountUrl(url: string, params?: string) {
    ...
    return `${API_BASE_URL}${url}?ts=${timestamp}${
      params || ''
    }&apikey=${PUBLIC_KEY}&hash=${hash.hex()}`;
  }

  async geHeros(name: string, currentOffset: number) {
    let params = '';

    if (name) {
      params += `&nameStartsWith=${name}`;
    }
    params += `&offset=${currentOffset}&orderBy=name&limit=4`;
